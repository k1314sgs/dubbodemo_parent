            ...
            {field: 'name', title: '区域名称', width: 100, align: 'left'},
            {
                ...
                formatter: function (value, row, index) {
                    return "<a href='javascript:deleteDistrict(" + row.id + ")'>删除</a>" +
                        "<a href='javascript:showDistrict(" + row.id + ")'>显示街道</a>";
                }
            }
        ]]
    });
...
function updateDistrict() {
    ...
    $.post("getDistrictById", {"id": selects[0].id}, function (data) {
        //回显
...
function deleteMoreDistrict() {
    ...
    if (selects.length == 0) {
        ...
        $.messager.confirm('删除框', '删除？？？？', function (r) {
            if (r) {
                $.post("deleteMoreDistrict", {"ids": arr.join()}, function (data) {
                    if (data.result == 0) {
                        $.messager.alert("系统提示", "删除失败");
                        return;
                    }
    ...
    $('#showTable').datagrid({
        ...
        columns: [[
            ...
            {field: 'name', title: '街道名称', width: 100, align: 'left'},
            {
                field: 'a', title: '操作', width: 100, align: 'left',
                formatter: function (value, row, index) {
                    return "<a href='javascript:deleteStreet(" + id + ")'>删除</a>";
                }
            }
        ]]
    });
